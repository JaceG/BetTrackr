...
### Data Storage Solutions & Business Model

**Free Tier (All Users):**
- **No Account:** All features available via localStorage (offline mode)
...
- User schema includes `stripeCustomerId` and `stripeSubscriptionId` fields for future use
...
**Database Schema:**
- User schema: _id (ObjectId), username (string), email (string), password (bcrypt hash), createdAt (Date), stripeCustomerId (optional), stripeSubscriptionId (optional)
...
- Tip Expenses: _id (string), date (string), amount (number), provider (string), notes (string)
...
- Available to all authenticated users (no subscription required)
...
### Free Cloud Sync & Storage Logic

**Storage Mode Determination (Updated):**
- `useCloudStorage` flag = `isAuthenticated` (all authenticated users get free cloud sync!)
- Unauthenticated users: `useCloudStorage = false` → localStorage only
- Authenticated users (free): `useCloudStorage = true` → MongoDB cloud sync ✅
- No subscription required for cloud storage - it's free for all users!

**MongoDB as Source of Truth:**
- When authenticated, MongoDB is the authoritative data source
- Load effect uses `hasLoadedFromMongo` flag to prevent initial race conditions
- After initial load, MongoDB data always syncs to local state
- CSV imports save directly to MongoDB when authenticated (via createEntry mutations)
- Manual entries immediately saved to MongoDB and reflected in UI

**Capital Injection Auto-Generation:**
- Auto-generation effect runs ONLY when `useCloudStorage = false` (localStorage users)
...
- Authenticated users: No auto-generation, server data is authoritative

**Data Synchronization:**
- **Betting Entries & Capital Injections:** MongoDB (when authenticated) or localStorage (when not)
- **Tip Expenses:** Always localStorage (no MongoDB backend yet)
- **Baseline & Week Settings:** MongoDB via settings API (when authenticated) or localStorage
- **Calculator Settings:** Always localStorage (app-level settings, not user data)

**CSV Import Flow:**
- **Authenticated:** CSV entries → createEntry mutations → save to MongoDB → query refetch → UI updates
- **Unauthenticated:** CSV entries → setEntries → save to localStorage → UI updates
- Tip expenses always saved to localStorage regardless of auth status

**Migration Banner:**
- Shows for authenticated users with localStorage data
- One-click migration: betting entries, capital injections, tip expenses, and settings → MongoDB
- Post-migration: localStorage cleared, cloud storage becomes authoritative

**API Access:**
- All authenticated users have free access to MongoDB cloud sync
- No subscription required for basic features
- Future premium features reserved for subscribers (Stripe integration ready)