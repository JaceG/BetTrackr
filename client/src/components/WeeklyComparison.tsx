...
function ComparisonStat({ 
  ...
  const percentChange = lastWeek !== 0 ? ((difference / Math.abs(lastWeek)) * 100) : (thisWeek !== 0 ? 100 : 0);
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-center min-w-[60px]">
          ...
          <p className="text-sm font-mono font-medium text-muted-foreground">
            {isCurrency ? (
              `${animatedLastWeek >= 0 ? '+' : '-'}$${Math.abs(animatedLastWeek).toLocaleString()}`
            ) : animatedLastWeek}
          </p>
        </div>
        ...
        <div className="text-center min-w-[60px]">
          ...
          <p className={`text-sm font-mono font-bold ${
            isCurrency 
              ? (thisWeek >= 0 ? 'text-profit' : 'text-loss')
              : 'text-foreground'
          }`}>
            {isCurrency ? (
              `${animatedThisWeek >= 0 ? '+' : '-'}$${Math.abs(animatedThisWeek).toLocaleString()}`
            ) : animatedThisWeek}
          </p>
        </div>
...
export default function WeeklyComparison({ entries }: WeeklyComparisonProps) {
  ...
  const hasData = thisWeekStats.betsPlaced > 0 || lastWeekStats.betsPlaced > 0;

  if (!hasData) {
    return (
      <Card className="p-4 sm:p-6 border-dashed bg-card/50">
        <div className="text-center text-muted-foreground">
          <p className="text-sm">No betting data from the past two weeks</p>
          <p className="text-xs mt-1">Start tracking bets to see weekly comparisons</p>
        </div>
      </Card>
    );
  }
