...
interface StatsStripProps {
	...
	bettingProfit: number; // Profit before tips are subtracted
...
function AnimatedStatCard({
	...
	const displayValue = `${prefix}$${Math.abs(
		animatedValue
	...
	if (featured) {
		return (
          ...
          ${
				isPositive
					? 'bg-gradient-to-br from-profit/5 via-profit/[0.02] to-transparent'
					: 'bg-gradient-to-br from-loss/5 via-loss/[0.02] to-transparent'
			}
          hover-lift transition-smooth`}>
				<div
					className={`absolute inset-0 opacity-30 ${
						isPositive
							? 'bg-gradient-to-br from-profit/10 to-transparent'
							: 'bg-gradient-to-br from-loss/10 to-transparent'
					}`}
				/>
				...
				<div className='relative flex items-center justify-between'>
					...
					<Icon
						className={`w-4 h-4 sm:w-5 sm:h-5 ${iconColorClass}`}
					/>
				</div>
				<p
	...
	return (
		<Card
			className={`p-2.5 sm:p-4 space-y-0.5 sm:space-y-2 ${borderClass} hover-lift transition-smooth`}>
			<div className='flex items-center justify-between'>
				...
				<Icon
					className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${iconColorClass}`}
				/>
			</div>
			<p
...
function WinRateDonut({
	...
	const animatedWinRate = useAnimatedCounter(winRate, {
		duration: 800,
		decimals: 1,
	});
...
export default function StatsStrip({
	...
	const isBettingProfitPositive = bettingProfit >= 0;
	...
	// Animated ROI value
	const animatedRoi = useAnimatedCounter(Math.abs(roi), {
		duration: 800,
		decimals: 1,
	});
		...
		<div className='space-y-3'>
								...
								<Tooltip>
									<TooltipTrigger asChild>
										<Button
											...
											size='icon'
											className='h-4 w-4 p-0'
											data-testid='info-win-rate'>
											<Info className='w-3 h-3 text-muted-foreground' />
										</Button>
									</TooltipTrigger>
									<TooltipContent className='max-w-xs'>
										<p>
											Percentage of winning bets out of
											total bets placed
										</p>
									</TooltipContent>
								</Tooltip>
			...
			<div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3'>
				<AnimatedStatCard
					...
					prefix=''
					tooltip='Your actual bankroll right now: Starting balance + deposits + betting profit/loss'
					icon={DollarSign}
					isNeutral={true}
				...
				<AnimatedStatCard
					...
					numericValue={bettingProfit}
					prefix={isBettingProfitPositive ? '+' : '-'}
					tooltip='Your profit from betting before tip costs are subtracted. This is how much you actually won/lost from bets.'
					icon={isBettingProfitPositive ? TrendingUp : TrendingDown}
					isPositive={isBettingProfitPositive}
					testId='betting-profit'
				/>
				...
				<AnimatedStatCard
					...
					prefix=''
					tooltip="The highest cash on hand you've reached during your betting history"
					icon={Target}
					isNeutral={true}
					testId='peak-balance'
				/>
