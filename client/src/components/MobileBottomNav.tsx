...
interface MobileBottomNavProps {
	...
	onScrollToSection: (
		section: 'stats' | 'chart' | 'history' | 'tips'
	) => void;
}
		...
		<>
			...
			{showAddMenu && (
				<div className='fixed bottom-20 left-1/2 -translate-x-1/2 z-50 sm:hidden animate-fade-in-up'>
					<div className='bg-card border border-border rounded-2xl shadow-xl p-2 flex flex-wrap gap-2 justify-center'>
						<Button
							onClick={() => {
								onAddEntry();
								setShowAddMenu(false);
			...
			<nav
				...
				<div className='relative flex items-center justify-around px-2 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]'>
					...
					<button
						...
						<Plus
							className={`w-6 h-6 ${
								showAddMenu ? 'text-foreground' : 'text-white'
							}`}
						/>
					</button>
