...
const crosshairPlugin: Plugin<'line'> = {
	...
	afterDatasetsDraw: (chart) => {
		...
		if (activeElements && activeElements.length > 0) {
			...
			const runningBalancePoint = activeElements.find(
				(el: any) => el.datasetIndex === 1
			);
...
export default function ChartCard({
	...
	const peak =
		runningBalances.length > 0 ? Math.max(...runningBalances) : baseline;
	const valley =
		runningBalances.length > 0 ? Math.min(...runningBalances) : baseline;
	...
	const stepSize =
		niceIncrements.find((inc) => inc >= rawStepSize) || rawStepSize;
	...
	const adjustedBaseline = [
		...
		...data.map((d) => {
			...
			if (d.running > currentYellow) {
				...
				outstandingInjections = Math.max(
					0,
					outstandingInjections - excess
				);
			}
			...
			},
			{
				...
				segment: {
					borderColor: (ctx: any) => {
						...
						return ctx.p1.parsed.y >= adjustedValue
							? 'hsl(142, 76%, 50%)'
							: 'hsl(0, 72%, 60%)';
					},
				},
			...
			annotation: {
				annotations: {
					baseline: {
						...
						label: {
							display: true,
							content: `Start: $${baseline.toLocaleString()}`,
							position: 'start',
	...
	const minChartWidth =
		effectiveDays > 7 ? `${effectiveDays * 30}px` : '100%';
	...
	const changePercent =
		baseline !== 0 ? ((changeAmount / baseline) * 100).toFixed(1) : '0';
		...
		<Card className='overflow-hidden border-border/50 bg-card/50 backdrop-blur-sm'>
			...
			<div className='px-2 sm:px-4 pb-4 sm:pb-6'>
				<div
					...
					<div
						className='h-[280px] sm:h-[380px] lg:h-[500px]'
