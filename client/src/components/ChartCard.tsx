...
export default function ChartCard({ data, baseline, capitalInjections = [] }: ChartCardProps) {
  ...
  // Track outstanding capital: injections that haven't been recouped yet
  ...
  const adjustedBaseline = [baseline, ...data.map((d) => {
    ...
    // Add injection (only once per unique datetime to avoid duplicates)
    ...
    // Yellow line = baseline + outstanding injections
