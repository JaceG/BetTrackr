...
export default function Home() {
  ...
  useEffect(() => {
    if (baseline !== null && entries.length > 0) {
      ...
      const previousAutoCount = existingInjections.filter(inj => inj.autoGenerated).length;
      if (autoGeneratedInjections.length > 0 && previousAutoCount === 0) {
        toast({
          title: "Capital Injections Calculated",
          description: `Generated ${autoGeneratedInjections.length} capital injection${autoGeneratedInjections.length === 1 ? "" : "s"} for existing entries`,
        });
      }
    }
