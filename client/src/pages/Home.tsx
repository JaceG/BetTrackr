...
export default function Home() {
	...
	const dataPoints = useMemo(() => {
		if (viewMode === 'per-day') {
			...
			for (const [dateKey, dayEntries] of Array.from(
				dailyGroups.entries()
			).sort()) {
				const totalNet = dayEntries.reduce((sum, e) => sum + e.net, 0);
				const totalBet = dayEntries.reduce(
					(sum, e) => sum + e.betAmount,
					0
				);
				const totalWinning = dayEntries.reduce(
					(sum, e) => sum + e.winningAmount,
					0
				);
				const notes = `${dayEntries.length} ${
					dayEntries.length === 1 ? 'bet' : 'bets'
				}`;

				const isFirstEntryInDay = dayEntries.some(
					(e) => e.id === firstEntryId
				);
				const firstEntry = dayEntries.find(
					(e) => e.id === firstEntryId
				);
	...
	return (
		<div className='min-h-screen bg-background'>
			...
			<ConfirmDialog
				...
				onConfirm={
					deleteId || deleteTipId ? confirmDelete : confirmClearAll
				}
				title={
					deleteId
						? 'Delete Entry?'
